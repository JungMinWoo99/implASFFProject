...
def GetVGGFaceFea(I_h, I_truth):
    def asff_output_to_vgg_input(asff_out):
        ...
        for i in range(asff_out.size(0)):
            vgg_input = torch.cat(
                (vgg_input, normalize(asff_out[i], VGGFaceModel.meta['mean'], VGGFaceModel.meta['std']).unsqueeze(0)),
                0)
        return vgg_input
...
def ASFFStyleLoss_test(I_h_fea_list, I_truth_fea_list):
    def ToGramMatrix(tensor):
        (b, n, h, w) = tensor.size()
        features = tensor.view(b, n, w * h)
        features_transpose = features.transpose(1, 2)
...
def ASFFGLoss_test(I_h, I_truth, I_h_D_output):
    normalized_I_h = normalize(I_h / 255.0, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
    normalized_I_truth = normalize(I_truth / 255.0, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
    resize_I_h = resize_transform(I_h)
    resize_I_truth = resize_transform(I_truth)
    I_h_fea_list, I_truth_fea_list = GetVGGFaceFea(resize_I_h, resize_I_truth)
    G_style_loss = ASFFStyleLoss_test(I_h_fea_list, I_truth_fea_list)
    G_perc_loss = ASFFPercLoss(I_h_fea_list, I_truth_fea_list)
    G_adv_loss = ASFFadvGLoss(I_h_D_output)
